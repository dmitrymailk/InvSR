...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument(
        "--chopping_bs", type=int, default=8, help="Batchsize for chopped patch"
    )
    parser.add_argument(
        "-t", "--timesteps", type=int, nargs="+", help="The inversed timesteps"
    )
    parser.add_argument(
        "-n", "--num_steps", type=int, default=1, help="Number of inference steps"
    )
    parser.add_argument(
        "--cfg_path",
        type=str,
        default="./configs/sample-sd-turbo.yaml",
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Path for Stable Diffusion Model",
    )
    parser.add_argument(
        "--started_ckpt_path",
        type=str,
        default="",
        help="Checkpoint path for noise predictor",
    )
    parser.add_argument(
        "--tiled_vae",
        type=str2bool,
        default="true",
        help="Enabled tiled VAE.",
    )
    parser.add_argument(
        "--color_fix",
        type=str,
        default="",
        choices=["wavelet", "ycbcr"],
        help="Fix the color shift",
    )
    parser.add_argument(
        "--chopping_size",
        type=int,
        default=128,
        help="Chopping size when dealing large images",
    )
